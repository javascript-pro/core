...
export const fetchGlobalNav = () => async (dispatch: TUbereduxDispatch, getState: any) => {
  try {
    ...
    const res = await fetch('/globalNav.json?t=' + Date.now(), { cache: 'no-store' });
    if (!res.ok) {
    ...
    dispatch(setUbereduxKey({ key: 'lastGlobalNavFetch', value: Date.now() }));

    console.log('fetchGlobalNav loaded fresh globalNav at', new Date().toISOString());
  } catch (e: unknown) {
