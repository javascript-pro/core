import { Object3D, MeshMatcapMaterial, MeshBasicMaterial, Color, BoxGeometry, Mesh } from "three";
...
const matCapMaterial = new MeshMatcapMaterial({ matcap: loader.resources.matcap.asset });
...
export default class Tile extends Object3D {
  ...
  async loadLOD(lodLevel, debug = false) {
    ...
    if (!this.lodCache[lodLevel]) {
      try {
        const glb = await loader.loadAsync(PATH_PREFIX + this.tileData.name + `_Buildings_LOD_${"1"}_draco.glb`);
    ...
    if (!this.terrainCache[lodLevel]) {
      try {
        const glbTerrain = await loader.loadAsync(PATH_PREFIX + this.tileData.name + `_Terrain_LOD_${'1'}_draco.glb`);
    ...
    Object.keys(this.lodCache).forEach(level => {
      this.lodCache[level].visible = (parseInt(level) === this.lodLevel) && this.lodLevel < 3;
